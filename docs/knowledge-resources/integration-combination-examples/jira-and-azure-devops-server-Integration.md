...
<code class="expression">space.vars.OIM</code> integrates Jira and Azure DevOps Server (TFS) bi-directionally. On this page, we will discuss the bi-directional integration of:
...
* In the **Add Project(s) to Sync** section, select the projects you want to synchronize between Jira and Azure DevOps Server (TFS) by clicking them. For example, here we select <code class="expression">space.vars.OIM</code> Demo Project from Jira and TestProject from Azure DevOps Server (TFS).  
