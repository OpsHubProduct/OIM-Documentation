<code class="expression">visitor.claims.unsigned.product</code> requires this service to communicate with the Azure DevOps. It acts as a translation layer between Azure DevOps and <code class="expression">visitor.claims.unsigned.product</code> and must be configured for synchronization with Azure DevOps.
...
* The <code class="expression">visitor.claims.unsigned.product</code> Service requires the machine to have .NET framework 4.7.2 or higher installed on it.
...
In case the machine on which <code class="expression">visitor.claims.unsigned.product</code> installed is behind the proxy (network proxy), then perform the steps mentioned in the [Proxy settings](../../manage/administrator/proxy-setting.md) section.

It is also required to configure the proxy settings for <code class="expression">visitor.claims.unsigned.product</code> Service, refer to [Proxy settings](../../connectors/azure-devops.md#proxy-settings-for-the-service) in appendix section for the <code class="expression">visitor.claims.unsigned.product</code> Service to learn the configuration steps. 