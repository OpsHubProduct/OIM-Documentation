...
* Create a user in TestRail that is dedicated for <code class="expression">space.vars.OIM</code>. Please click [here](#testrailadduser) to see how you can add a user in TestRail.
...
* From the Test Rail version **8.0.3 (Default)** release, there are changes in its Attachment API. Because of the misbehavior in the Attachment API, attachment might not synchronize properly through <code class="expression">space.vars.OIM</code>.
...
* For TestRail as a source/target system:
  * Currently, the below mentioned formatting is supported for **Markdown fields**, and the rest formatting will not synchronize via <code class="expression">space.vars.OIM</code>:
    * Bold
    * Italics
    * Ordered list
