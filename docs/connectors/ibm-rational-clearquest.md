...
* Create one user of IBM Rational ClearQuest dedicated to <code class="expression">space.vars.OIM</code>. This user shouldn't be able to do any operation from system's user interface.
...
For <code class="expression">space.vars.OIM</code> to integrate with IBM Rational ClearQuest, IBM Rational ClearQuest team server must be installed and running.
...
* Restart <code class="expression">space.vars.OIM</code> server after copy.
...
To integrate, <code class="expression">space.vars.OIM</code> requires few queries to be created in IBM Rational ClearQuest that can be accessed by the integration user only.
...
Below are the sample snippets of how the JSON query can be used as the default link query in <code class="expression">space.vars.OIM</code>.
...
* **Add/Edit Workflow transition XML in Mapping Configuration of <code class="expression">space.vars.OIM</code>**
...
* **Change State Transition Configuration in IBM Rational ClearQuest**
  * IBM Rational ClearQuest allows to change **State Transition** configuration. Use this configuration to configure any-to-any transition(s). Workflow transition mapping is not required to be configured in <code class="expression">space.vars.OIM</code> if any-to-any state transition is configured.
  * <code class="expression">space.vars.OIM</code> will be able to perform direct transition of the state field to the desired state as this option works when any-to-any transition is configured for an integration user.
...
* The target lookup query for IBM Rational ClearQuest must be in a specific JSON format defined by <code class="expression">space.vars.OIM</code> due to a particular IBM Rational ClearQuest API behaviour. Refer to section [How to Create Look Up Json Query](clearquest.md#look-up-json-query) for details on particular JSON format.
...
* For fields that are showing read only in mapping of <code class="expression">space.vars.OIM</code>, it is possible that such fields' Default Behavior is configured as read-only in Field Behavior section of ClearQuest Designer.
  * Such fields are writeable if they are marked mandatory for any state in ClearQuest Designer, then they can be provided as dependent fields in Transition XML in <code class="expression">space.vars.OIM</code>.
...
* Only basic authentication is supported in <code class="expression">space.vars.OIM</code>.
* Comments will be synchronized only for the default **Notes** fields in IBM Rational ClearQuest end system.
  ...
  * If any other comment field enabled with different name, those will not be part of comments synchronization of <code class="expression">space.vars.OIM</code>.
