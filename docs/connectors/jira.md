...
* When Jira instance is 'On Demand'. Please refer [this](https://confluence.atlassian.com/cloud/api-tokens-938839638.html) link for generating Jira API Token.
  * In case you are using scoped API token, then you need to provide required scops:
    * When Jira is source system:
    ...
    * When Jira is a target system:
  
      | **Scope Name**                                   | **Scope Type** |
      |--------------------------------------------------|----------------|
      | write:jira-work                                  | Classic        |
      | write:sprint:jira-software                       | Granular       |
      | All the scopes of _"When Jira is source system"_ |                |
  * A Jira scoped API token automatically expires after the duration selected during its creation.
...
* If a scoped API token is used in a Jira Cloud system, the Sprint field on Issues will not sync because the required API is not available for scoped tokens.
