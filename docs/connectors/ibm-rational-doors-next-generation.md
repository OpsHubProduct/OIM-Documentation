...
* Create one user of DOORS Next system, dedicated to <code class="expression">space.vars.OIM</code>. Refer to section [Create User](#how-to-create-a-user) for steps on creating a user. This user should not be used to do any operations from end system's user interface.  
  Following are the privileges required by the dedicated integration user for synchronization of the artifacts by <code class="expression">space.vars.OIM</code>.
...
## <code class="expression">space.vars.OIM</code> behaviour

* **Projects in <code class="expression">space.vars.OIM</code>**
  * For <code class="expression">space.vars.OIM</code>, **Streams** are considered as Projects. Hence user can configure bidirectional integration with any other end system with context of DOORS Next streams and the other system's projects.
  * The Projects in <code class="expression">space.vars.OIM</code> are shown as a combination of **Project, Component and Stream**. All the streams are displayed in a hierarchy of its parent stream.
    * **For example -**  
      If there exists a project "ProjectA" with component "ComponentA" and stream "StreamA", then in <code class="expression">space.vars.OIM</code>, the project is shown as a hierarchy of "ProjectA/ComponentA/StreamA".  
    ...
    > **Note**: The streams in DOORS Next are considered as projects. Thus, considering the above example, there will be 3 projects in <code class="expression">space.vars.OIM</code> - **ProjectA/ComponentA/StreamA**, **ProjectA/ComponentA/StreamA/StreamA.1** and **ProjectA/ComponentA/StreamA/StreamA.2**.

* **Synchronization in <code class="expression">space.vars.OIM</code>**
  ...
  * By default, <code class="expression">space.vars.OIM</code> synchronizes both core artifacts and shared artifacts present in stream.
    * **For example:** There exists a core artifact E1 in a stream. The artifact E1 is added in two modules "Module 1" and "Module 2". Then there will be two shared artifacts created. One in "Module 1" and the other in "Module 2".
      ...
      > **Note**: In this case, <code class="expression">space.vars.OIM</code> will treat and synchronize all the 3 artifacts as individual entities.

  * <code class="expression">space.vars.OIM</code> provides a provision to restrict the synchronization to only shared artifacts i.e. synchronize only the shared artifacts present in the stream.
    ...
    * **Restrict writing of artifacts to DOORS Next module(s)** - <code class="expression">space.vars.OIM</code> supports writing of any artifact in a specific module. This is done through field mapping in DOORS Next system. The field purpose is to decide which module is used to create the shared artifacts. Refer to **Field - Module** section of [Fields available in DOORS Next](#fields-available-in-doors-next).
...
Along with above fields, there are few fields which are added by <code class="expression">space.vars.OIM</code> in fields mapping for synchronization purpose. Following are the list of fields and the synchronization behavior when these fields are mapped.

* **Field Name : In Folder**  
  * In DOORS Next, the artifacts are organized in folders.  
  * In <code class="expression">space.vars.OIM</code>, when DOORS Next is a source system, **"In Folder"** is used to represent the folder path of artifacts being synchronized.  
  * In <code class="expression">space.vars.OIM</code>, when DOORS Next is a target system, **"In Folder"** is used to represent the folder path where the artifacts needs to be created.  
  * The datatype of this field in <code class="expression">space.vars.OIM</code> is **Hierarchy**. The value mappings for this is displayed in a tree structure. For more information on value mapping, refer to document [Value mapping](mapping-configuration#value-mapping)  
...
* **Field Name : Module**  
  ...
  * In <code class="expression">space.vars.OIM</code>, when DOORS Next is a source system, **"Module"** is used to represent the module name of artifacts being synchronized.  
  ...
  * In <code class="expression">space.vars.OIM</code>, when DOORS Next is a target system, **"Module"** is used to represent the module name where the artifacts needs to be created.  
  * The datatype of this field in <code class="expression">space.vars.OIM</code> is **Hierarchy**. The value mappings for this field is displayed in a tree structure. For more information on value mapping, refer to document [Value mapping](mapping-configuration#value-mapping)  
  * The top level represents the stream path. The modules of the stream gets loaded when the stream path is expanded.  
  * **For example -** If there are two modules "Module 1" and "Module 2" in stream, then in value mapping, there will be two modules displayed under that stream - "Module 1", "Module 2". Refer to below image for the value mapping on <code class="expression">space.vars.OIM</code> UI.
...
* **Field Name : DOORS Next Project**  
  ...
  * This is a read-only field in <code class="expression">space.vars.OIM</code>.
...
Given below are the sample snippets of how the OpsHub query can be used as default link query in <code class="expression">space.vars.OIM</code>. For the examples below, the id information of the source system entity id is stored in source_system_id.
...
This issue can be resolved by adding **Workflow Transition** XML in mapping configuration of <code class="expression">space.vars.OIM</code>.
...
With this option, <code class="expression">space.vars.OIM</code> makes the required intermediate status transition automatically as per the transition(s) configuration in the end system.
...
When DOORS Next is a source system, <code class="expression">space.vars.OIM</code> supports synchronization of data from specific module(s).  
...
Given below are the sample snippets of how the JSON query can be used as target entity lookup query in <code class="expression">space.vars.OIM</code>.  
...
- DOORS Next API returns the comment's updated time in a different format as per the end system version or setting of the end system's advance property `RDMPublishUseLocaleForCommentsTimeStamp`.  
  <code class="expression">space.vars.OIM</code> uses this default format(s): `[MMMM d, yyyy at hh:mm:ss aaa z, yyyy-MM-dd'T'HH:mm:ss.SSSZ]` for date parsing.  
  ...
  due to <code class="expression">space.vars.OIM</code> being unable to parse unknown format.

- Configure the **Comment Date Format** input on the integration level to override the default date format used by <code class="expression">space.vars.OIM</code> with the date format corresponding to the date format of updated time to resolve the parsing error.
...
- Hence, to avoid such failures, this setting can be used. Select `Yes` value in below section of advanced settings:
  - The `Override parameters for read operations` of the `Entity level advance` setting of integration in <code class="expression">space.vars.OIM</code>, when IBM Doors Next is source system in <code class="expression">space.vars.OIM</code>  
  - The `Override parameters for write operations` of the `Entity level advance` setting of integration in <code class="expression">space.vars.OIM</code>, when IBM Doors Next is target system in <code class="expression">space.vars.OIM</code>
...
- **Scenario 1** - There are few revisions in artifacts of `Stream A.1`. Those revisions will get synchronized in the entities of target system (entities in the target project corresponding to `Stream A.1`).  
  When `Stream A.1` gets merged to `Stream A`, then a single revision gets added in artifacts of `Stream A`. <code class="expression">space.vars.OIM</code> will synchronize that revision and subsequent revisions of artifacts in `Stream A` to the entities in target project corresponding to `Stream A`.
...
- **Scenario 2** - Now consider, there are few revisions in artifacts of `Stream A.2`. As `Stream A.2` is not configured for synchronization, the artifacts of `Stream A.2` will not get synchronized.  
  ...
  <code class="expression">space.vars.OIM</code> will synchronize that revision and any other subsequent revisions of artifacts in `Stream A` to the entities in target project corresponding to `Stream A`.
...
- With respect to the above end system behavior, following will be the synchronization behavior.
  ...
  - Any new comment and link added in the artifact will be synchronized by <code class="expression">space.vars.OIM</code>.
...
- With respect to the above behavior, <code class="expression">space.vars.OIM</code> creates two revisions for **Primary Text** field, one for update and other one for the file (attachment) added as inline-image.
...
- Based on the module field value mapping and DOORS Next system behavior, <code class="expression">space.vars.OIM</code> will also create the core artifact in the folder associated with the module (each module has their separate asset folder associated) and then the created artifact will be added in the corresponding module as a shared artifact.

> **Note**: For above case, when the **Module** field is mapped and once the artifact is created by <code class="expression">space.vars.OIM</code> in target DOORS Next system, then any further updates for the corresponding entity from source system will be written in the artifact belonging to module (shared artifact).
...
* **Configuration Management**  
  ...
  * <code class="expression">space.vars.OIM</code> supports synchronization in local configuration scope. Synchronization of Global configuration streams are not supported by <code class="expression">space.vars.OIM</code>. Thus, cross component links are not supported for DOORS Next by <code class="expression">space.vars.OIM</code>.
...
* Comments added in artifacts during merge from another stream will not be synchronized by <code class="expression">space.vars.OIM</code>.
...
* Synchronization of the inline document is not supported for HTML field.
  ...
  * If the Source field, which is mapped with the HTML field of Doors Next contains the hyperlink to the inline file, then below will be the synchronization behavior:
    ...
    * If the above-mentioned data is synchronized from Doors Next to another end system via <code class="expression">space.vars.OIM</code>, then synchronization will generate data corruption.
...
- **Generate OAuth Token and OAuth Secret for given consumer key**  
  ...
  - [Option 1 (Utility)](ibm-rational-doors-next-oauth-generation.md#generate-token-using_doorsng-token-generator-utility): Generate the OAuth token using a stand-alone utility bundled with <code class="expression">space.vars.OIM</code> to generate the OAuth token.
...
- Suppose the token mentioned in the System Configuration form in the <code class="expression">space.vars.OIM</code> gets expired, then it must be updated by reperforming the above steps.
- To avoid the above case, the value of the "OAuth access token timeout" configuration would be larger than the synchronization interval (Integration schedule) of the integration, which is configured with DOORS Next using OAuth authentication.
	- If the configured timeout is not large enough, and the token is not used by <code class="expression">space.vars.OIM</code> in the configured timespan for some reason (Inactive integration/Inactive <code class="expression">space.vars.OIM</code> server), then token will get expired.
...
* **Stream** – The word **Stream** refers to Projects in terms of <code class="expression">space.vars.OIM</code>. For more understanding on streams, refer to document [DOORS Next Streams](https://www.ibm.com/support/knowledgecenter/SSYMRC_6.0.6/com.ibm.jazz.vvc.doc/topics/t_wkspc_crt.html).
