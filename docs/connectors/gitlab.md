...
<code class="expression">space.vars.OIM</code> Integration Configuration requires Personal Access Token to authenticate with GitLab's API.  
...
Before you proceed to the integration, you must first configure GitLab onto <code class="expression">space.vars.OIM</code>.  
...
<code class="expression">space.vars.OIM</code> supports "Epic" entity of Group.

### In case of Project

<code class="expression">space.vars.OIM</code> supports two entities of Project: Commit Information, Issues (Incident, Requirements).
...
* Severity, Health Status fields are supported as read only in <code class="expression">space.vars.OIM</code>.
  * Reason: Write API unavailability.
* Milestones and Iterations sync is not supported.
* Synchronization of User fields:
  * For the user sync, the advanced mapping needs to be configured in the <code class="expression">space.vars.OIM</code> where the excel based look-up can be used for user mapping.
...
* GitLab as a source system:
  * <code class="expression">space.vars.OIM</code> cannot synchronize commits older than 1 year for a specific project.
