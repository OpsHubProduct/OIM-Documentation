...
For integration, <code class="expression">space.vars.OIM</code> requires user that is authorized to connect to Subversion server URL and can read check-in information.
...
Before you continue to the integration, you must first configure Subversion onto <code class="expression">space.vars.OIM</code>.  
...
* `url`:= WSDL url of OpsHubwebservice. Edit localhost by <code class="expression">space.vars.OIM</code> server name/server ip.
...
* `commitOnConnectionFailure`:= True, if it makes SCM working even in the absence of <code class="expression">space.vars.OIM</code> server, otherwise False.
