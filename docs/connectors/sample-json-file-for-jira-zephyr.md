...
* Additional fields need to be included in the JSON for Test Plans:
  * At the field level: Add the system-specific field requestFieldKey in the system-specific map for all custom fields.
      * This is used by <code class="expression">space.vars.OIM</code> to update Test Plan custom fields via a private API.
...
* At the entity level: Add the system-specific field readCustomFieldsMeta in the metadata to define whether <code class="expression">space.vars.OIM</code> can use the private API to load custom field information.
