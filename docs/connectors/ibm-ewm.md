...
* Create one user of Engineering Workflow Management (EWM), dedicated to <code class="expression">space.vars.OIM</code>. User should not be used to do any operations from system's user interface.
...
* **Approvals** field is supported as Read-only in <code class="expression">space.vars.OIM</code>.
...
* Wiki type of field is supported as Read-only text field.
  * The list of vulnerabilities depends on <code class="expression">space.vars.OIM</code> version, your EWM version, and its system configuration.
...
> * Suppose the token mentioned in the System Configuration form in the <code class="expression">space.vars.OIM</code> gets expired, then it must be updated by re-performing the above steps.
