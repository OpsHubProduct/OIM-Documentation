...
* Lookup Select Box and Lookup Multiple Choice variables are not supported because they can be linked to dynamic fields that can reference any column.
* Masked variables are not supported because the API does not return decrypted values for these fields. As a result, the actual stored data cannot be retrieved or processed.
* Catalog variable history is supported only for the Requested Item entity; for other entities such as Catalog Task, the API does not provide history, and variables are synchronized in their current state only.
...
```json
{
  ...
  "CATALOG_VARIABLE_CONFIG": {
    ...
    "apiDetails": {
      ...
      "GET_CATALOG_VARIABLE_CHOICES": {
        "apiUrl": "<api_url_to_be_appended_to_the_base_instance_url>",
        "methodType": "GET"
      },
      "GET_CATALOG_FROM_REQUESTED_ITEM": {
        "apiUrl": "<api_url_to_be_appended_to_the_base_instance_url>",
        "methodType": "GET",
        "queryParams": {
