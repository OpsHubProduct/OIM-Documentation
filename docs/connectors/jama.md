...
*Create one user in Jama that is dedicated to <code class="expression">space.vars.OIM</code>. This user should not do any operations from the system's interface.*  
...
* To validate the permission of the user being used in the <code class="expression">space.vars.OIM</code>, please refer to [Access Permission of Jama users](#access-permission-of-jama-users) section.  
...
* **Lock State Field**  
  * To denote the status of lock on the item, **OH Lock Status** field is introduced in **<code class="expression">space.vars.OIM</code>**.  
...
- This field must be mapped in <code class="expression">space.vars.OIM</code> for integration to synchronize the sets.  
- Jama doesn't provide any revision when Set Key is updated. Due to this, the field in <code class="expression">space.vars.OIM</code> also works on current state.  
...
- After the Soft Delete operation is performed by <code class="expression">space.vars.OIM</code> in Jama, the entity will be deleted in Jama. The deleted entity can be found in the "dashboard" of the corresponding project.  
...
- Jama allows to organize the items in tree structure. To synchronize the items maintaining the tree structure, below configurations need to be performed in <code class="expression">space.vars.OIM</code>.  
...
**Known Limitations:**  
- When Jama is configured as the source system in <code class="expression">space.vars.OIM</code>:
...
However, you don't need to write any criteria query in <code class="expression">space.vars.OIM</code>. Instead, you need to give the filter id for the same.  
...
- When Test Steps are not present in the Test Run, then the Status field can be updated by <code class="expression">space.vars.OIM</code>. Otherwise Status field is set according to the status of the steps present.  
...
- When Jama is Source system and integration is configured with Default settings [Sync Only Current State = Select/No](../integrate/integration-configuration.md#sync-only-current-state):
  - From Jama, the revisions information comes as a string which generally follows a predefined format. <code class="expression">space.vars.OIM</code> is internally parsing the revisions string to extract the relevant old and new values for each field from this revision string.
...
- Jama will automatically move the child entities associated with that **Component** or **Set**, which is moved to the new project by <code class="expression">space.vars.OIM</code>, regardless of whether those child entities are configured within <code class="expression">space.vars.OIM</code>.
...
- Select the user to be mentioned in <code class="expression">space.vars.OIM</code> from "Users";
