...
13. **Sync Queued:** The entity is queued for synchronization.
14. **Sync Started:** Synchronization of the entity has started.

> **Note:** The **Sync Queued** and **Sync Started** statuses are applicable only for integrations with systems for which <code class="expression">space.vars.SITENAME</code> supports batch writing. To view queued changes, refer to [Viewing Queued Entities for Synchronization](integration-sync-report.md#viewing-queued-entities-for-synchronization).

## Viewing Queued Entities for Synchronization

<code class="expression">space.vars.SITENAME</code> temporarily stores changes queued for batch writing in its database to ensure consistency and reliable processing. Follow the steps below to view them:
