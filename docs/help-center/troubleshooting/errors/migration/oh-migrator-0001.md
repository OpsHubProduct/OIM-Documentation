...
This error primarily occurs when <code class="expression">space.vars.OIM</code> is not able to establish connection to the database that was used to configure  
...
Please check the Migration.log file located under the logs folder in the <code class="expression">space.vars.OIM</code> installation path. This file contains all the step wise details of migration process. The exact cause of the failure should be found at the end of the file.
