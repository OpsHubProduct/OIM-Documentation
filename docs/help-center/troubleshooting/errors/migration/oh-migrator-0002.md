...
Following are the steps to recover from the failure, actual solution will be provided by <code class="expression">space.vars.OIM</code> support team.

**Send error details to <code class="expression">space.vars.OIM</code> Support**

These error details will be needed by support team to investigate the failure and provide solution.

1. Copy the data from <code class="expression">space.vars.OIM</code> installation folder to another location.  
2. Share Migration.log file with <code class="expression">space.vars.OIM</code> support. This file will be located at `<installation_path>/logs` folder.

**Restore the application to earlier state**

Since the migration has failed, <code class="expression">space.vars.OIM</code> may not be in proper state to function. If you want to restore  
