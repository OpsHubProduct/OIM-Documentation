...
**Case 3:** While creating an entity of Sub-Task type, there is not even one parent entity that is recognized by <code class="expression">space.vars.OIM</code> found in the target Jira system. This parent entity will be known to <code class="expression">space.vars.OIM</code> only if this entity has been polled or created through some integration in <code class="expression">space.vars.OIM</code>.
...
**Solution 2.**  
For the case in **Possibility 2** part of the **Cause**, there needs to be exactly one parent relationship for the Sub-Task. It is an ambiguous case for <code class="expression">space.vars.OIM</code> and it cannot take the decision to which parent relationship should be selected for the Sub-Task.  
