...
If we have a state transition defined for a field, but related details of Workflow transition are not provided in <code class="expression">space.vars.OIM</code>, then this error occurs.

## Solution

In <code class="expression">space.vars.OIM</code> field mapping configuration, we need to provide the state transition in given format. Refer to [Workflow Transition](../../../../integrate/mapping-configuration.md#workflow-transition) for details.
