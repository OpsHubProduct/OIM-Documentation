...
<code class="expression">space.vars.OIM</code> synchronizes events in a sequential order from the source system to maintain proper track of changes. Due to this reason, if any failure already exists for an entity, then all the further events for the same entity will go in failure queue with this error.
