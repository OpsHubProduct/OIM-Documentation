...
For some entity types, having a Parent/Dependent entity is required to create the entity in end System. With the same alignment, when <code class="expression">space.vars.OIM</code> is trying to create an entity in such system, the <code class="expression">space.vars.OIM</code> would expect the Parent/Dependent entity should be available. In case such Parent/Dependent entity is not present/found in the target end system, user would receive such processing failure.
