...
* In the bidirectional integration between systems A and B, before <code class="expression">space.vars.OIM</code> processed the sync, the entity was updated in system B first when the entity was trying to sync to system A. In such a case, it leads to data inconsistency in the sync.  
* To prevent this data inconsistency, <code class="expression">space.vars.OIM</code> blocks the processing of the entity.
