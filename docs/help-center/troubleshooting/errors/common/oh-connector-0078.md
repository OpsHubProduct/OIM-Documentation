...
When a Web Link is mapped with Standard Link, then incoming Web Link for synchronization in <code class="expression">space.vars.OIM</code> will have limited details like link URL and link title. Standard Linkages are established based on issues that are synchronized by <code class="expression">space.vars.OIM</code>. As Web Link does not have enough information to establish the Standard Linkage, this combination is not supported.
