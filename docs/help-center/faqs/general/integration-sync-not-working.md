...
* Check which user has created the entity in system A. If the user used for configuring system A in <code class="expression">space.vars.OIM</code> has done the changes in system A, then those changes will be skipped by <code class="expression">space.vars.OIM</code> for synchronization. The user used in configuring system in <code class="expression">space.vars.OIM</code> is a reserved user for <code class="expression">space.vars.OIM</code>. If you want to validate synchronization, then create/update entity with a user other than this user in system A and then wait for the synchronization.
* Check the failed event - There can be failures logged in <code class="expression">space.vars.OIM</code> due to which entities are not synchronized in destination system. For managing failed events, refer [Manage Integration Failures](../../troubleshooting/manage-integration-failures.md).
...
* Check both systems are up. Make sure that both the systems are up and reachable from the machine on which <code class="expression">space.vars.OIM</code> is hosted.
...
* Check the polling time in integration. 'Start polling time' means the time from when <code class="expression">space.vars.OIM</code> should start polling data from the source system. For more information, refer [Polling Time usage in Integration](start-polling-time-usage-in-integration.md).
* Check the Sync Report. This will provide the information for each entity synchronized by <code class="expression">space.vars.OIM</code>. For more information on how to see sync report, refer [Integration Sync Report](../../troubleshooting/integration-sync-report.md).
