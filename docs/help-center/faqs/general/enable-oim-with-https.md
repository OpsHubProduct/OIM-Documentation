...
<code class="expression">space.vars.OIM</code> has been installed with HTTP. However, even after installation with a few steps, the user can enable it to connect with HTTPS. 

# Solution

Follow the steps given below to enable HTTPS on <code class="expression">space.vars.OIM</code> which has already been installed with HTTP:
...
* Following are the commands that need to be executed in the given sequence within the Command Prompt on the machine where <code class="expression">space.vars.OIM</code> has been installed:  
...
5. To access <code class="expression">space.vars.OIM</code> with HTTPS, please refer to the [Configurations for HTTPS](#configurations-for-https) section.  
6. Save the `server.xml` file.  
7. Start the OpsHub Server.  
8. Access <code class="expression">space.vars.OIM</code> with the URL:  
