...
A failed event on a given entity is blocking further synchronization. For example, if <code class="expression">space.vars.OIM</code> notices that for entity E1, there is already a failed event in the queue, then all further updates will fail with the above error.  
...
* Go to <code class="expression">space.vars.OIM</code> Dashboard and click on the Integration Name:  
