...
* Login to `<Server_URL>/OpsHubWS`  
  ...
  **Note:** If you are accessing <code class="expression">space.vars.OIM</code> from a different machine, then change the localhost to the IP address or hostname of the machine where <code class="expression">space.vars.OIM</code> is installed.
...
* Login to <code class="expression">space.vars.OIM</code> through `<Server_URL>/OIM`  
