...
I have <code class="expression">space.vars.OIM</code> installed on one machine. Now, I want to move <code class="expression">space.vars.OIM</code> from that machine to other machine. How can I do this migration?
...
2. You have the password of systems users configured in <code class="expression">space.vars.OIM</code>.  
3. These steps are applicable for same database type, for example, if your <code class="expression">space.vars.OIM</code> database is on MySQL, then you can move it to other instance of MySQL (not to any other database type).  
4. Make sure your old and new database versions are also same. For example, if your <code class="expression">space.vars.OIM</code> is installed on MySQL 5.6, then your new database MySQL instance should also have that same version.  
...
6. If for any reason, installation failed, follow the steps below:  
   ...
   * If you are not able to resolve this error, then send the `<Install_Folder>/logs` folder to <code class="expression">space.vars.OIM</code> support team. Restore application to earlier state until this issue is fixed:
