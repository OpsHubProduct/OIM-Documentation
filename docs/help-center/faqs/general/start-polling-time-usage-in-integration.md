...
'Start Polling Time' means the time from when <code class="expression">space.vars.OIM</code> should start polling data from the source system. <code class="expression">space.vars.OIM</code> fetches only those changes/entities that are updated after given 'Start Polling Time'.
