...
## Cause
If <code class="expression">space.vars.OIM</code> is able to decide whether the source or target Jira field is of type text or wiki then <code class="expression">space.vars.OIM</code> would be able to handle the formatting by itself. But the formatting cannot be handled by default by <code class="expression">space.vars.OIM</code> as the Jira API doesn't provide the information whether a text field is a simple text field or wiki type of field.
