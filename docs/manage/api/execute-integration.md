...
* Let's say the <code class="expression">space.vars.OIM</code> is installed on 10.13.27.211 instance on HTTP and user credentials for this instance is “admin” and “password”.
* A user has configured the integration on the <code class="expression">space.vars.OIM</code>, which is having id 2 and this integration is in the Active mode.
